import { OutputSafetyEvaluation, ResponsibilityEvaluation, RiskFactor } from '../types';
...
export class ResponsibilityEngine {
  ...
  public evaluateOutput(output: string): { evaluation: OutputSafetyEvaluation; riskFactors: RiskFactor[] } {
    const result = this.evaluate(output);
    const unsafe = result.evaluation.injectionDetected || result.evaluation.brandSafetyViolation;
    return {
      evaluation: {
        evaluated: true,
        piiDetected: result.evaluation.piiDetected,
        injectionDetected: result.evaluation.injectionDetected,
        toxicityScore: result.evaluation.toxicityScore,
        sanitizedOutput: result.evaluation.sanitizedPrompt,
        engineStatus: unsafe ? 'BLOCKED' : result.evaluation.piiDetected ? 'INTERVENED' : 'PASSED',
        reason: unsafe
          ? `Unsafe content detected in downstream output: ${result.evaluation.reason}`
          : result.evaluation.piiDetected
            ? 'Sensitive entities were redacted from the downstream output.'
            : 'Downstream output passed deterministic safety checks.'
      },
      riskFactors: result.riskFactors.map(factor => ({
        ...factor,
        description: `Postflight output check: ${factor.description}`
      }))
    };
  }
}
