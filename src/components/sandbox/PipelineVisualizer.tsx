...
export const PipelineVisualizer: React.FC = () => {
  ...
  const stagesToRender: RuntimeStageState[] = isExecuting && activeStages.length > 0 
    ? activeStages 
    : (currentEvent?.pipelineStages && currentEvent.pipelineStages.length > 0 
        ? currentEvent.pipelineStages 
        : [
            { stage: 'RECEIVED', name: 'Request Received & Normalization', engine: 'CORE', status: 'PASSED', durationMs: 2.4, detail: 'Payload verified. Tenant: Acme Corp.' },
            { stage: 'PII_CHECK', name: 'PII & Sensitive Data Detection', engine: 'RESPONSIBILITY', status: 'PASSED', durationMs: 3.8, detail: 'Aadhaar, SSN & token inspection.' },
            { stage: 'INJECTION_GUARD', name: 'Prompt Injection & Jailbreak Defense', engine: 'RESPONSIBILITY', status: 'PASSED', durationMs: 4.2, detail: 'Structural heuristic safety checks.' },
            { stage: 'POLICY_EVAL', name: 'Enterprise Policy Matching', engine: 'CORE', status: 'PASSED', durationMs: 2.1, detail: 'Evaluated against active compliance rules.' },
            { stage: 'RISK_SCORING', name: 'Composite Risk Scoring', engine: 'CORE', status: 'PASSED', durationMs: 1.9, detail: 'Synthesized 3-engine risk factor vector.' },
            { stage: 'CACHE_LOOKUP', name: 'Semantic Cache & Similarity Lookup', engine: 'COST', status: 'PASSED', durationMs: 3.1, detail: 'Vector index similarity check.' },
            { stage: 'ROUTING', name: 'Dynamic Model Routing', engine: 'COST', status: 'PASSED', durationMs: 1.8, detail: 'Complexity-based tier optimization.' },
            { stage: 'INFERENCE', name: 'LLM Inference Execution', engine: 'CORE', status: 'PASSED', durationMs: 380, detail: 'Executed downstream model request.' },
            { stage: 'HALLUCINATION_CHECK', name: 'Hallucination & Faithfulness SLM Verification', engine: 'PERFORMANCE', status: 'PASSED', durationMs: 5.6, detail: 'Grounding validation against reference facts.' },
            { stage: 'DECISION_FINAL', name: 'Final Governance Decision & Audit Dispatch', engine: 'CORE', status: 'PASSED', durationMs: 1.5, detail: 'Decision synthesized & audit logged.' }
          ]);
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-bold text-zinc-200 uppercase tracking-wider">
            Real-Time 10-Stage Runtime Pipeline
          </h2>
        </div>
