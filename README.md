...
16. [2-Minute Hackathon Walkthrough](#2-minute-hackathon-walkthrough)
...
│   ├── repository.ts                    # SQLite persistence helpers for policies, events, and reviews
│   └── provider.ts                      # Optional OpenAI-compatible downstream adapter
│
└── src/
...
* **`RuntimeEvent`**: Complete telemetry object capturing `requestId`, `timestamp`, `application`, `model`, `routedModel`, `rawInput`, `sanitizedInput`, `rawOutput`, `finalOutput`, `inputSha256`, `outputSha256`, `responsibility`, optional postflight `outputSafety`, `cost`, `performance`, `riskScore`, `riskFactors`, `decision`, `triggeredPolicies`, and `latency`.
...
| :--- | :--- | :--- | :--- |
| `SQLITE_DB_PATH` | No | SQLite database path for policies, runtime events, and review queue data. Defaults to `controlplane.sqlite`. | `./data/controlplane.sqlite` |
| `OPENAI_BASE_URL` | No | Base URL for an OpenAI-compatible `/chat/completions` endpoint. | `https://api.openai.com/v1` |
| `OPENAI_API_KEY` | No | API key sent to the configured downstream provider. | `...` |
| `OPENAI_MODEL` | No | Downstream model name. All three `OPENAI_*` values must be set to enable inference. | `gpt-4o-mini` |

When the OpenAI-compatible settings are absent, the gateway performs preflight governance only and does not make a downstream call. It returns an explicit no-provider response; token, provider inference latency, and cost remain unmeasured rather than simulated. With a provider configured, the pipeline runs preflight responsibility checks, calls the provider when not blocked, then runs postflight output safety and performance checks. Provider-reported usage is used for token accounting and cost is calculated only when usage and a known model price (or provider-reported cost) are available. SQLite is initialized with the default policies on first run and survives server restarts.
